...
class Othello:
    ...
    """
    def __init__(self, discs=[[3, 3, 1], [3, 4, 2], [4, 3, 2], [4, 4, 1]],offset=10, tile_size=80, line_width=4, line_color=(0,0,0), background_color=(255,255,255), disc_radius=30, disc_color_1=(0,0,255), disc_color_2=(255,165,0), A=[], active_player=1, legal_moves=[], players=['human', 'computer'], strategy=[],zone_corner=[[[0, 0], [7, 0], [0, 7], [7, 7]]], zone_red=[[1, 0], [0, 1], [1, 1], [6, 0], [6, 1], [7, 1], [0, 6], [1, 6], [1, 7], [6, 6], [6, 7], [7, 6]]):
        ...
        self.zone_corner = zone_corner
        self.zone_red = zone_red
    ...
    def strategy0(self):
        ...
        for i in range(len(self.legal_moves)):
            discs = self.legal_moves[i][2]
            L.append(discs)
        minimum_discs = min(L)  #finds the minimum amount of disc the computer can take.
        #print('Min: ',minimum_discs)
        choices = []
        for i in range(len(self.legal_moves)):
            if self.legal_moves[i][2] == minimum_discs:
                choices.append(self.legal_moves[i])
        choice = random.choice(choices)  #random choicee from minimum disc moves
        #print('computer chooses: ',choice)
        col = choice[0]
        row = choice[1]
    ...
    def strategy1(self):
        ...
        for move in self.legal_moves:
            col = move[0]
            row = move[1]
            if [col, row] in self.zone_corner:
                print('Taking the corner')
                return col, row
            else:
                'No corner moves available'
        ...
        for move in self.legal_moves:
            ...
            row = move[1]
            captured = move[2]
            if [col, row] not in self.zone_red:  #list of legal moves not in the red_zone
                moves.append([col, row, captured])
        print(moves)
        if len(moves) == 0:
            self.strategy0()  #call the disc-minimizing move
        else:  # If the list moves did not remain empty (great!) then replace the contents of legal_moves with it, again call strategy0() to select from legal_moves the disc-minimizing move, and return the result.
            self.legal_moves = []
    ...
    def strategy2(self):
        ...
        col = choice[0]
        row = choice[1]
        print('Active player', self.active_player, 'using strategy2(). Temporarily the same as strategy0().') 
        return col, row
...
R.set_player_strategy(1, 'intermediate')
R.set_player_strategy(2, 'beginner')
R.start()